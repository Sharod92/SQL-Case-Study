...
SELECT SUM(votes) AS german_votes
FROM GERMAN_LANG_MOVIES;


WITH ITALIAN_LANG_MOVIES 
AS
(
SELECT SUM(total_votes) AS votes
FROM movie AS m
...
SELECT SUM(votes) AS italian_votes
FROM ITALIAN_LANG_MOVIES;
...
SELECT *
FROM ranked_production_company
...
ORDER BY actor_rank, r.total_votes DESC;
...
FROM duration_summary WINDOW w1 AS (
...
AS (
	SELECT g.genre
		...
		,DENSE_RANK() OVER (
			PARTITION BY m.year ORDER BY m.worlwide_gross_income DESC
			) AS movie_rank
	FROM movie AS m
	INNER JOIN genre AS g ON m.id = g.movie_id
	WHERE genre IN (
