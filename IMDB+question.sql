...
WITH ranked_movies
AS(
	SELECT mov.title, rat.avg_rating, DENSE_RANK() OVER (ORDER BY rat.avg_rating DESC) AS movie_rank
	FROM movie AS mov
		INNER JOIN
		ratings AS rat
...
(
	SELECT	n.name AS director_name,
		...
		DENSE_RANK() OVER(ORDER BY COUNT(d.movie_id) DESC) director_rank
	FROM names AS n
		...
		INNER JOIN genre AS g
			USING (movie_id),
			top_3_genres AS t3g
	WHERE r.avg_rating > 8 AND g.genre IN (t3g.genre)
	GROUP BY n.name
	ORDER BY movie_count DESC
)
...
WITH ranked_production_company
AS(
...
SELECT * FROM ranked_production_company
...
SELECT n.name AS actor_name,
    ...
    DENSE_RANK() OVER(ORDER BY ROUND(SUM(avg_rating*total_votes)/SUM(total_votes),2) DESC, r.total_votes) AS actor_rank
FROM names AS n
